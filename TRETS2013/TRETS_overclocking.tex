...
\begin{figure}[htbp]
        \centering
        \subfigure[\scriptsize The modeled value and the experimental results of the maximum word-length of RCA and CSA.]{
...
\begin{table*}[htbp]
 ...
 %% increase table row spacing, adjust to taste
   ...
   \tbl{Relative Reduction of MRE ($\%$) in Overclocking Scenario for Various Normalized Frequencies Based on~(\ref{Relative Reduction MRE}) for two types of input data: the uniform data (Uni) and real image data from Lena (Lena).}{
   \centering
   \footnotesize
   %\vspace{-4mm}
...
Whilst this research has shown a promising direction for sustaining performance scaling, there are several issues that would be of interest for future research. A critical finding from this work is the direction inconsistency between carry propagation (from LSB to MSB) and results sampling (from MSB to LSB). In this case, although the probability of generating overclocking errors might be small, the magnitude of error is large. Another limitation of performance improvement is that the inputs are applied in parallel, while the correct outputs are updated based on a bit-serial fashion due to the delay of carry propagation.  To tackle this problem, we wish to investigate non-conventional arithmetic operations and data representations. One possible candidate is known as ``online arithmetic '' [xxx], in which the computation is performed bit by bit from MSB to LSB, and carry propagation can be nearly removed because a redundant number system [xxx] is used to represent numbers. In this case, we would expect an even more significant performance improvement when applying our approach into the aforementioned area.
