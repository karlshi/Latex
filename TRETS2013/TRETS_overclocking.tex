...
\begin{abstract}
%As process scaling introduces significant performance variations, much more conservative timing margins will have to be applied to avoid timing violations. Potentially, r
Releasing the tight accuracy requirement would potentially offer greater freedom to create a design with better performance or energy efficiency. In this paper, we compare two different approaches that could trade accuracy for performance. One is the traditional approach where the precision used in the datapath is limited to meet a target latency. The other is a proposed new approach which simply allows the datapath to operate without timing closure. We demonstrate analytically and experimentally that on average our approach obtains either smaller errors for equivalent faster operating frequencies in comparison to the traditional approach. This is because the worst case caused by timing violations only happens rarely, while precision loss results in errors to most data. We also show that for basic arithmetic operations such as addition, applying our approach to the simple building block of ripple carry adders can achieve better accuracy or performance than using faster adder designs to achieve similar latency.
 ...
 %operators (i.e. ripple carry adders) can achieve better accuracy or performance than using the traditional method to both original operators and intrinsically faster operators (i.e. carry select adders) which consumes the same silicon area.
\end{abstract}

\category{C.2.2}{Category put here}{Detail category}

\terms{Design, FPGA, Performance}

\keywords{Overclocking, Imprecise Design}
...
%and  that are commonly used in DSP applications and numerical algorithms.
...
\begin{figure}[t]
    ...
    \vspace{-3ex}
    \subfigure[]{
        \includegraphics[width=.50\textwidth]{./Figures/CSA2stage_Model_Timing2.eps}
        \label{Fig_CSA2stage_Timing}
    }%
    \subfigure[]{
        \includegraphics[width=.5\textwidth]{./Figures/CSA3stage_Area2.eps}
        \label{Fig_CSA3stage_Area}
    }
    \vspace{-2ex}
...
%in comparison to the RCA, the CSA achieves greater word-length for relatively relexed frequency constraints. The RCA only outperforms when very high frequency requirement is applied. This is because at low frequencies, although the multiplexer delay limits the word-length of each stage in CSA when compared to RCA, the stage parallelism in CSA enables a greater word-length. However when more stringent latency requirement is applied, the word-length of each stage in CSA is limited. Hence the multiplexer delay becomes comparable to the delay of the carry chain, and this inhibits the benefits of parallelism.
...
\vspace{-1ex}
 %% increase table row spacing, adjust to taste
   ...
   \tbl{Relative Reduction of MRE ($\%$) in Overclocking Scenario for Various Normalized Frequencies Based on~(\ref{Eq:Relative_Reduction_MRE}) for Two Types of Input Data: the Uniform Data (Uni) and Real Image Data from Lena (Lena).}{
   \centering
   %\footnotesize
   \scriptsize
   %\vspace{-4mm}
...
%In general, we can observe a relatively small area overhead of our approach, especially 
...
%frequency is initially increased or with a small error budget (i.e. $\sim10\%$).
%
\begin{table*}[tbp]
\vspace{-2ex}
 %% increase table row spacing, adjust to taste
...
%explore the probabilistic behaviour of alternative 
