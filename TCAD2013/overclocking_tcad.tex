...
\begin{eqnarray}\label{CSA_SingleStageDelay}
  d_i=n_i\cdot \mu_{c}+xxx\cdot\mu_{mux} 
...
Substituting (\ref{CSA_SingleStageDelay}) into (\ref{CSA_DelayEqual}) yields (), which denotes the word-length of stage $i$.
...
\begin{eqnarray}\label{CSA_Timing}
  n_{CSA}=cn_{RCA}-\frac{\mu_{mux}}{\mu_{c}}\cdot\frac{(c+2)(c-1)}{2}
\end{eqnarray}

%In addition, the word-length of the least significant stage should be greater than 0. Therefore
In addition, combining (\ref{CSA_DelayRep}) and (\ref{CSA_RCA}) to ensure that $n_{c-1}>0$. Therefore the stage number is bounded by (\ref{CSA_StageNumber}).
...
\subsection{Comparison between CSA and RCA}
