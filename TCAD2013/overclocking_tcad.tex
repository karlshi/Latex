...
\author{Kan~Shi,
        ...
        and~George~A.~Constantinides,~\IEEEmembership{Senior~Memberellow,~IEEE}% <-this % stops a space
...
Although advanced architectures such as CSA inheritly offer better performance than the basic structure, this would generate a large area overhead. In the following experiments, the trade-offs between accuracy, performance as well as area are explored. If the available hardware resources are limited, the full word-length of both CSA and RCA might not be implemented. Therefore truncation errors would be generated. For instance, the number of available LUTs is set to 45, 35, 25 and 15 respectively. The corresponding error expectations of RCA and CSA are demonstrated in Fig.xxx. The optimal design method which achieves the minimum error expectation is labelled.
\begin{figure}[htbp]
	\centering
...
%  \label{Fig Tradeoffs}
%\end{figure*}
%the maximum word-lengths of RCA and CSA under given area constraints are listed in Table.xxx


It can be seen that if area is not limited, CSA with higher stage numbers serve as the optimal design choice for most frequency values, as we discussed in Section.xxx. While overclocking the RCA achieves better accuracy at higher frequencies, d
%According to 
